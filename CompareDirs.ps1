...
$f2 = Get-ChildItem -Exclude $exc -Include $inc  -Recurse -path D:\dev\CyberScope\trunk\CSwebdev\code\CyberScope | WHERE-OBJECT{ $_.LastWriteTime -gt '5/15/2020' }

Compare-Object -ReferenceObject $f1 -DifferenceObject $f2 -Property Name,   LastWriteTime 

 

Get-ChildItem -Exclude $exc  -Recurse -path D:\dev\CyberScope\trunk\CSwebdev\code\CyberScope* | `
